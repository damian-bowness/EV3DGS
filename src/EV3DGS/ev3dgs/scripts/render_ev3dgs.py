...
class EV3DGSOptions:
    filter3D: bool = False
    """Enable 3D mip-style filtering by modifying opacities/scales based on depth and focal length."""

    filter3D_scale: float = 0.2
    """Strength of the 3D filter. Higher values apply stronger depth-dependent smoothing."""

    two_pass: bool = True
    """Enable EV3DGS two-pass filtering: run a first pass to score Gaussians, then suppress low-quality contributions in the final render."""

    xg_thresh: float = 0.01
    """Two-pass threshold for gradient-sensitivity score (xg). Gaussians below this score are filtered out in the second pass."""

    cc_thresh: float = 0.5
    """Two-pass threshold for contribution ratio (cc). Gaussians with contribution ratio below this value are filtered out in the second pass."""
...
def _render_trajectory_video(
    ...
    # Add EV3DGS parameters
    ev3dgs: Optional[EV3DGSOptions] = None
            ...
            for camera_idx in progress.track(range(cameras.size), description=""):
                ...
                if crop_data is not None:
                    ...
                    with torch.no_grad():
                        ...
                        #    cameras[camera_idx : camera_idx + 1], obb_box=obb_box
                        #)
                        pipeline.model.filter3D       = ev3dgs.filter3D       
                        pipeline.model.two_pass       = ev3dgs.two_pass       
                        pipeline.model.filter3D_scale = ev3dgs.filter3D_scale     
                        pipeline.model.xg_thresh      = ev3dgs.xg_thresh          
                        pipeline.model.cc_thresh      = ev3dgs.cc_thresh
                        pipeline.model.rastFlag       = not ev3dgs.two_pass          

                        cam = cameras[camera_idx]
                        raster_pkg = get_render_output(pipeline.model, cam, True)
                        outputs = {}
                        outputs["rgb"] = raster_pkg.view(3,cam.height.item(),cam.width.item()).permute(1,2,0).clamp(max=1) 
...
class BaseRender:
    ...
    image_format: tyro.conf.Suppress[Literal["jpeg", "png"]] = "jpeg"
    """Image format"""
    jpeg_quality: tyro.conf.Suppress[int] = 100
    """JPEG quality"""
    downscale_factor: tyro.conf.Suppress[float] = 1.0
    """Scaling factor to apply to the camera image resolution."""
    eval_num_rays_per_chunk: tyro.conf.Suppress[Optional[int]] = None
    """Specifies number of rays per chunk during eval. If None, use the value in the config file."""
    rendered_output_names: tyro.conf.Suppress[List[str]] = field(default_factory=lambda: ["rgb"])
    """Name of the renderer outputs to use. rgb, depth, etc. concatenates them along y axis"""
    depth_near_plane: tyro.conf.Suppress[Optional[float]] = None
    """Closest depth to consider when using the colormap for depth. If None, use min value."""
    depth_far_plane: tyro.conf.Suppress[Optional[float]] = None
    """Furthest depth to consider when using the colormap for depth. If None, use max value."""
    colormap_options: tyro.conf.Suppress[colormaps.ColormapOptions] = colormaps.ColormapOptions()
    """Colormap options."""
    render_nearest_camera: tyro.conf.Suppress[bool] = False
    """Whether to render the nearest training camera to the rendered camera."""
    check_occlusions: tyro.conf.Suppress[bool] = False
    """If true, checks line-of-sight occlusions when computing camera distance and rejects cameras not visible to each other"""
    ev3dgs: EV3DGSOptions = field(default_factory=EV3DGSOptions)
            ...
            with Progress(
                ...
                for camera_idx, (camera, batch) in enumerate(progress.track(dataloader, total=len(dataset))):
                    with torch.no_grad():
                        ...
                        pipeline.model.filter3D       = self.ev3dgs.filter3D       
                        pipeline.model.two_pass       = self.ev3dgs.two_pass       
                        pipeline.model.filter3D_scale = self.ev3dgs.filter3D_scale          
                        pipeline.model.xg_thresh      = self.ev3dgs.xg_thresh           
                        pipeline.model.cc_thresh      = self.ev3dgs.cc_thresh
                        pipeline.model.rastFlag       = not self.ev3dgs.two_pass             
                        camera.camera_to_worlds = camera.camera_to_worlds.squeeze() # splatoff rasterizer requires cam2world.shape = [3,4]
                        raster_pkg = get_render_output(pipeline.model, camera, True)
                        outputs = {}
                        outputs["rgb"] = raster_pkg.view(3,camera.height.item(),camera.width.item()).permute(1,2,0).clamp(max=1) 
