import torch
from ev3dgs import GaussianRenderSettings, GaussianRender
...
# taken from gaussian-opacity-fields (gof)
...
def get_render_output(model, camera, renderer=False):
    background = torch.ones(3, device=model.device) # get_background(model) - use white background to compare with bayes rays # model.background 
    ...
    # TODO: get rid of subpixel offset and scale_modifier
    raster_settings = GaussianRenderSettings(
        image_height= height,
        image_width= width,
    ...
    render = GaussianRender(raster_settings=raster_settings)
    ...
    rastFlag = model.rastFlag
    two_pass = model.two_pass
    xg_thresh = model.xg_thresh/1000.0
    cc_thresh = model.cc_thresh
