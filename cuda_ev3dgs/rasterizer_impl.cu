...
__host__ __device__ __forceinline__ int getHigherMsb(uint32_t n)
{
#if defined(__CUDA_ARCH__)
	return (n == 0) ? 0 : (32 - __clz(n));
#else
...
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char *&chunk, size_t P)
{
	...
	obtain(chunk, geom.quadricCoeffs, P * 10, 128);
...
void CudaRasterizer::Rasterizer::rasterizeForward(
	const dim3 block,
	...
	bool debug)
{
	...
	duplicateWithKeys<<<(P + 255) / 256, 256>>>(
		...
		CHECK_CUDA(, debug)
			...
			const uint32_t tile_count = static_cast<uint32_t>(tile_grid.x) * static_cast<uint32_t>(tile_grid.y);
	int bit = getHigherMsb(tile_count);
	...
	float *dCount;
	const size_t need_bytes = static_cast<size_t>(P) * sizeof(float);
	cudaMalloc(&dCount, need_bytes);
	cudaMemsetAsync(dCount, 0, need_bytes, 0);

	if (two_pass)
	{
		float *dReject;
		cudaMalloc(&dReject, need_bytes);
		cudaMemsetAsync(dReject, 0, need_bytes, 0);

		CHECK_CUDA(FORWARD::gradient(
					   tile_grid, block,
					   imgState.ranges,
	...
	if (rastFlag)
	{
		CHECK_CUDA(FORWARD::renderRasterization(
					   dCount,
					   two_pass, cc_thresh,
					   tile_grid, block,
					   imgState.ranges,
	...
	{
		// NOTE: REPLACED COV3D WITH ROTATIONS
		CHECK_CUDA(FORWARD::renderRayMarch(
					   dCount,
					   tile_grid, block,
					   imgState.ranges,
					   binningState.point_list,
