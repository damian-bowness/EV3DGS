...
def populate_render_tab(
    ...
    render_button = server.gui.add_button(
        ...
        hint="Generate the ev3dgs-render command for rendering the camera path.",
    )
    ...
    def _(event: viser.GuiEvent) -> None:
        ...
        with event.client.gui.add_modal("Render Command") as modal:
            ...
            if control_panel is not None:
                if control_panel.two_pass:
                    extra_flags += ["--ev3dgs.two-pass", str(control_panel.two_pass)]
                    extra_flags += ["--ev3dgs.xg-thresh", str(control_panel.xg_filter)]
                    extra_flags += ["--ev3dgs.cc-thresh", str(control_panel.cc_filter)]
                else:
                    extra_flags += ["--ev3dgs.two-pass", str(control_panel.two_pass)]
            ...
            command_parts = [
                ...
                "--camera-path-filename", str(json_outfile.absolute()),
                "--output-path", f"renders/{dataname}/{render_name_text.value}.mp4",
                *extra_flags,
            ]
